...
<div class="content">
	<div class="body-container" style="width: 700px; margin: 10px auto;">
		<div class="body-title">
			<h3>
				<span style="font-family: Webdings">2</span> 게시판
			</h3>
		</div>
				...
				<c:forEach var="dto" items="${list}">
					<tr align="center" bgcolor="#ffffff" height="35"
						...
						<td align="left" style="padding-left: 10px;"><a
							href="${articleUrl}&num=${dto.num}">${dto.subject}
								(${dto.replyCount})</a></td>
						<td>${dto.userName}</td>
						<td>${dto.createDate}</td>
						<td>${dto.hitCount}</td>
						<td><c:if test="${not empty dto.saveFilename}">
								<a href="<%=cp%>/bbs/download?num=${dto.num}"><img
			...
			<table style="width: 100%; margin: 10px auto; border-spacing: 0px;">
				<tr height="40">
					...
					<td align="right" width="100">
						<button type="button" class="btn"
							onclick="javascript:location.href='<%=cp%>/sale/bbs/created';">글올리기</button>
					</td>
				</tr>
			</table>
		</div>
